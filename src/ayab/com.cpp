...
void Com::h_reqInit(const uint8_t *buffer, size_t size) {
  if (size < 3U) {
    ...
    send_cnfInit(static_cast<uint8_t>(ERR_EXPECTED_LONGER_MESSAGE));
    return;
  }
  ...
  if (crc8 != CRC8(buffer, 2)) {
    send_cnfInit(static_cast<uint8_t>(ERR_CHECKSUM_ERROR));
    return;
  }
...
void Com::h_reqStart(const uint8_t *buffer, size_t size) {
  if (size < 5U) {
    ...
    send_cnfStart(static_cast<uint8_t>(ERR_EXPECTED_LONGER_MESSAGE));
    return;
  }
  ...
  if (crc8 != CRC8(buffer, 4)) {
    send_cnfStart(static_cast<uint8_t>(ERR_CHECKSUM_ERROR));
    return;
  }
...
void Com::h_reqTest(const uint8_t *buffer, size_t size) {
  if (size < 2U) {
    // message is too short
    send_cnfTest(static_cast<uint8_t>(ERR_EXPECTED_LONGER_MESSAGE));
    return;
  }
